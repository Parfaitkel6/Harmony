...
document.addEventListener('DOMContentLoaded', () => {
    ...
    audioFileInput.addEventListener('change', async (e) => {
        ...
        // Prompt user to select target playlist name for routing path
        const playlistNames = customPlaylists.map(p => p.name).join(', ');
        let targetPlaylistName = prompt(`Enter target playlist name to save imported audio files into:\nAvailable: ${playlistNames}`, customPlaylists[0]?.name);
        
        let targetPlaylist = customPlaylists.find(p => p.name.toLowerCase() === (targetPlaylistName || '').trim().toLowerCase());
        if (!targetPlaylist) {
            targetPlaylist = customPlaylists[0]; // Fallback to first playlist if not found
        }

        for (const file of files) {
            try {
                // Route storage path specifically to user's playlist directory structure
                const storageRef = ref(storage, `users/${currentUser.uid}/playlists/${targetPlaylist.name}/${Date.now()}_${file.name}`);
                alert(`Uploading "${file.name}" to playlist path: ${targetPlaylist.name}...`);
                ...
                // Route and push track directly into target custom playlist array
                if (!targetPlaylist.tracks.some(t => t.title === newTrack.title)) {
        ...
        renderCustomPlaylists();
        updateMenuCounts();
        alert(`Successfully imported audio files and saved them to playlist: "${targetPlaylist.name}"!`);
